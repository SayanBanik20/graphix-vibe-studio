...
export const Route = createFileRoute("/shop")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse handcrafted photo frames, keepsakes, love books, and gift hampers from Graphix Vibe's Mumbai studio." },
      { property: "og:title", content: "Shop — Graphix Vibe" },
      { property: "og:description", content: "Handcrafted keepsakes, personalized gifts, and brand goods from Graphix Vibe." },
    ],
  }),
  component: Shop,
});

const categories = ["All", "Personalized Gifts", "Gift Hampers", "Branding", "Accessories"] as const;
      ...
      <section className="border-b border-border">
        <div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
          ...
          <p className="mt-6 max-w-xl text-muted-foreground">
            Every piece is designed in-studio and finished by our craftspeople. Made-to-order — usually within 3 days.
          </p>
        </div>
      </section>
            ...
            {categories.map((c) => (
              <button
                ...
                className={`rounded-full border px-4 py-2 text-sm transition-colors ${
                  active === c ? "border-foreground bg-foreground text-background" : "border-border bg-background hover:bg-muted"
                }`}
              >
                {c}
              </button>
